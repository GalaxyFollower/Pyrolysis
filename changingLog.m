function changingLog(Tinf, kinetics,r,vter_mult)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% INITIAL CONDITIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
global param
...
odeoptions=odeset('Stats','on','OutputFCn', ...
    @(t,X,init)plotPreDataAtStep(t,ri,X,param,init,hdl,plotem),'NonNegative',1);

[t X]=ode15s(@(t,X)massAndEnergyBalances(t,X,ri,param),([ 0 param.tf]),X0,odeoptions);
X0=X(end,:)';
...
odeoptions=odeset('Stats','on','OutputFCn', ...
    @(t,X,init)plotDataAtStep(t,ri,X,param,init,hdl,plotem),...
    'NonNegative',1);
...
while mean(X0(1:nv+1))>0.01*param.rhob0
    try
        ...
        [tlog X]=ode15s(@(t,X)massAndEnergyBalancesLOGP(t,X,ri,param),log([ t0 tf]),X0,odeoptions);

        timeLog=[timeLog;tlog(2:end)];
        XX=[XX;X(2:end,:)];
        if(tlog(end)<log(tf))
            succes=false;
        end
